...
#define RCC_Periph_GPIO_SCL  			RCC_APB2Periph_GPIOB  //Bus for GPIO Port of SCL
#define RCC_Periph_GPIO_SDA  			RCC_APB2Periph_GPIOB  //Bus for GPIO Port of SCL
...
#define GPIO_SCL                  GPIO_LCD
#define GPIO_SDA                  GPIO_LCD
...
void I2C_LowLevel_Init(void) {
  ...
  RCC_APB2PeriphClockCmd(RCC_Periph_GPIO_SCL | RCC_Periph_GPIO_SDA, ENABLE);
...
//	GPIO_AFConfigure(AFIO_I2C1_NO_REMAP);
		...
		GPIO_PinAFConfig(GPIO_LCD, GPIO_PinSource7, GPIO_AF_I2C1);
		GPIO_PinAFConfig(GPIO_LCD, GPIO_PinSource6, GPIO_AF_I2C1);
	#endif
#endif
  ...
  GPIO_Init(GPIO_LCD, &GPIO_InitStructure);
	...
	/*
	GPIO_SetBits(GPIO_LCD, GPIO_Pin_SCL|GPIO_Pin_SDA);
	(void) GPIO_ReadInputDataBit(GPIO_LCD, GPIO_Pin_SCL|GPIO_Pin_SDA);
	GPIO_ResetBits(GPIO_LCD, GPIO_Pin_SCL|GPIO_Pin_SDA);
	(void) GPIO_ReadInputDataBit(GPIO_LCD, GPIO_Pin_SCL|GPIO_Pin_SDA);
	GPIO_SetBits(GPIO_LCD, GPIO_Pin_SCL|GPIO_Pin_SDA);
*/
...
static uint32_t I2C_Start(void) {
  ...
  //Generate a start condition. (As soon as the line becomes idle, a Start condition will be generated)
	...
	while(ERROR ==I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_MODE_SELECT));
...
//	while(RESET == I2Cx->SR1&I2C_SR1_SB);
